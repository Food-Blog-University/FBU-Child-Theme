    ...
    switch ($bloglayout) {
        ...
        case "blogstyle1" :
            echo '<header class="entry-header">';
                ...
                echo '<h1 class="entry-title">'; if ( is_sticky() && is_home() && ! is_paged() ) : echo '<span class="featured-post">'; _e( 'Featured', 'pure-and-simple' );
                echo '</span>'; endif;
                echo '<a href="' .esc_url( get_permalink() ) .'" title="'; the_title();
                echo '">'; if(the_title( '', '', false ) !='') the_title(); else _e('Untitled', 'pure-and-simple');
                echo '</a></h1>';
            ...
            echo '</div></header><div class="entry-content">';
                ...
                $excerpt = get_theme_mod( 'excerpt_limit', '50' );
                     switch ($excon) {
                        case "content" :
                            the_content(__('Read More', 'pure-and-simple'));
                        break;
                        case "excerpt" :
                            echo '<p>' . puresimple_excerpt($excerpt) . '</p>' ;
                            echo '<p class="more-link"><a href="' . get_permalink() . '">' . __('Read More', 'pure-and-simple') . '</a>' ;
                        break;
                }
            ...
            echo '</div><footer class="entry-footer">';
                ...
                if( get_theme_mod( 'hide_edit' ) == '') {
                    edit_post_link( __( 'Edit this Post', 'pure-and-simple' ), '<span class="edit-link">', '</span>' );
                }
            echo '</footer>';
        break;
